...
![](数据结构与算法.assets/12.png)

![](数据结构与算法.assets/13.png)
...
**常见的时间复杂度对应的图:**![](数据结构与算法.assets/38.png)
...
![](数据结构与算法.assets/50.png)

![](数据结构与算法.assets/51.png)
...
![](数据结构与算法.assets/52.png)

![](数据结构与算法.assets/53.png)
...
![](数据结构与算法.assets/56.png)

![57](数据结构与算法.assets/57.png)

![58](数据结构与算法.assets/58.png)
...
2) 将折半查找中的求 mid 索引的公式 , low 表示左边索引 left, high 表示右边索引 right. key 就是前面我们讲的 findVal![](数据结构与算法.assets/61.png)

3) int mid = low + (high - low) * (key - arr[low]) / (arr[high] - arr[low]) ; 插值索引
对应前面的代码公式：
int mid = left + (right – left) * (findVal – arr[left]) / (arr[right] – arr[left])
4) 举例说明插值查找算法 1-100 的数组![](数据结构与算法.assets/62.png)
...
斐波那契查找原理与前两种相似，仅仅改变了中间结点（mid）的位置，mid 不再是中间或插值得到，而是位于黄金分割点附近，即 mid=low+F(k-1)-1（F 代表斐波那契数列），如下图所示![](数据结构与算法.assets/63.png)
...
3) 但顺序表长度 n 不一定刚好等于 F[k]-1，所以需要将原来的顺序表长度 n 增加至F[k]-1。这里的k 值只要能使得 F[k]-1 恰好大于或等于 n 即可，由以下代码得到,顺序表长度增加后，新增的位置（从n+1 到F[k]-1 位置），都赋为 n 位置的值即可。
while(n>fib(k)-1)
k++;
...
![](数据结构与算法.assets/64.png)

![](数据结构与算法.assets/65.png)
...
2) 二叉树的子节点分为左节点和右节点
3) 示意图![](数据结构与算法.assets/71.png)



4) 如果该二叉树的所有**叶子节点**都在**最后一层**，并且结点总数= **2^n -1**, n 为层数，则我们称为满二叉树。
   ![](数据结构与算法.assets/72.png)



5) 如果该二叉树的所有**叶子节点**都在**最后一层**或者**倒数第二层**，而且最后一层的叶子节点在左边连续，倒数第二层的叶子节点在右边连续，我们称为完全二叉树![](数据结构与算法.assets/73.png)
...
4) 完成删除思路分析![](数据结构与算法.assets/76.png)
...
![](数据结构与算法.assets/79.png)

**思路分析**: 中序遍历的结果：{8, 3, 10, 1, 14, 6}

![](数据结构与算法.assets/80.png)
...
![](数据结构与算法.assets/81.png)

5) 小顶堆举例说明

![](数据结构与算法.assets/82.png)
...
![](数据结构与算法.assets/87.png)

2) 重新调整结构，使其继续满足堆定义

![](数据结构与算法.assets/88.png)

3) 再将堆顶元素 8 与末尾元素 5 进行交换，得到第二大元素 8.

![](数据结构与算法.assets/89.png)

4) 后续过程，继续进行调整，交换，如此反复进行，最终使得整个序列有序

![](数据结构与算法.assets/90.png)
...
3) 树的带权路径长度：树的带权路径长度规定为**所有叶子结点的带权路径长度之和**，记为WPL(weighted path length) ,权值越大的结点离根结点越近的二叉树才是最优二叉树。

4) WPL 最小的就是赫夫曼树

![](数据结构与算法.assets/91.png)
...
2) 使用赫夫曼编码来生成赫夫曼编码数据 ,即按照上面的赫夫曼编码，将"i like like like java do you like a java"字符串生成对应的编码数据, 形式如下1010100010111111110010001011111111001000101111111100100101001101110001110000011011101000111100101000101111111100110001001010011011100
...
2. 具有以下特点：它是一 棵**空树**或**它的左右两个子树的高度差的绝对值不超过 1**，并且**左右两个子树都是一棵平衡二叉树**。平衡二叉树的常用实现方法有红黑树、AVL、替罪羊树、Treap、伸展树等。
3. 举例说明, 看看下面哪些 AVL 树, 为什么?

![](数据结构与算法.assets/100.png)
...
3) 举例说明(下面 2-3 树就是一颗多叉树)![](数据结构与算法.assets/105.png)
...
![](数据结构与算法.assets/113.png)

5) 有向图
6) 带权图

![](数据结构与算法.assets/114.png)
...
3. 举例说明
   ![116](数据结构与算法.assets/116.png)



### 3. 图的快速入门案例

1) 要求: 代码实现如下图结构![](数据结构与算法.assets/117.png)
...
2) 查找结点 v 的第一个邻接结点 w。
3) 若 w 存在，则继续执行
4) 如果 w 不存在，则回到第 1 步，将从 v 的下一个结点继续。
5) 若 w 未被访问，对 w 进行深度优先遍历递归（即把 w 当做另一个 v，然后进行步骤123）。
6) 查找结点 v 的 w 邻接结点的下一个邻接结点，转到步骤 3。
7) 分析图![](数据结构与算法.assets/118.png)
...
5) 算法的主要思想，利用动态规划来解决。每次遍历到的第 i 个物品，根据 w[i]和v[i]来确定是否需要将该物品放入背包中。即对于给定的 n 个物品，设 v[i]、w[i]分别为第 i 个物品的价值和重量，C 为背包的容量。再令v[i]、[j]表示在前 i 个物品中能够装入容量为 j 的背包中的最大价值。则我们有下面的结果：
...
![](数据结构与算法.assets/123.png)

2. 重复第一步，还是不符合，再后移

![](数据结构与算法.assets/124.png)

3. 一直重复，直到 Str1 有一个字符与 Str2 的第一个字符符合为止

![](数据结构与算法.assets/125.png)

4. 接着比较字符串和搜索词的下一个字符，还是符合。

![](数据结构与算法.assets/126.png)

5. 遇到 Str1 有一个字符与 Str2 对应的字符不符合。

![127](数据结构与算法.assets/127.png)
...
![](数据结构与算法.assets/128.png)

7. 怎么做到把刚刚重复的步骤省略掉？可以对 Str2 计算出一张《部分匹配表》，这张表的产生在后面介绍

![](数据结构与算法.assets/129.png)

8. 已知空格与 D 不匹配时，前面六个字符”ABCDAB”是匹配的。查表可知，最后一个匹配字符B对应的”部分匹配值”为 2，因此按照下面的公式算出向后移动的位数：
...
1) 假设存在如下表的需要付费的广播台，以及广播台信号可以覆盖的地区。 如何选择最少的广播台，让所有的地区都可以接收到信号![](数据结构与算法.assets/137.png)

2) 思路分析:
   如何找出覆盖所有地区的广播台的集合呢，使用穷举法实现,列出每个可能的广播台的集合，这被称为幂集。假设总的有 n 个广播台，则广播台的组合总共有
   2ⁿ -1 个,假设每秒可以计算 10 个子集， 如图:![](数据结构与算法.assets/138.png)
...
![](数据结构与算法.assets/142.png)



#### (4)普里姆算法最佳实践(修路问题)

![](数据结构与算法.assets/143.png)
...
![](数据结构与算法.assets/145.png)

例如，对于如上图 G4 所示的连通网可以有多棵权值总和不相同的生成树。

![](数据结构与算法.assets/146.png)
...
![](数据结构与算法.assets/147.png)

![](数据结构与算法.assets/148.png)

![](数据结构与算法.assets/149.png)

![](数据结构与算法.assets/150.png)

![](数据结构与算法.assets/151.png)

![](数据结构与算法.assets/152.png)
...
#### (6)如 何 判 断 是 否 构 成 回 路 -举 例说明(如图)

![](数据结构与算法.assets/153.png)
...
1) 马踏棋盘算法也被称为骑士周游问题
2) 将马随机放在国际象棋的 8×8 棋盘 Board[0～7]的某个方格中，马按走棋规则(马走日字)进行移动。要求每个方格只进入一次，走遍棋盘上全部 64 个方格
3) 游戏演示: http://www.4399.com/flash/146267_2.htm

![](数据结构与算法.assets/162.png)
